...
| **Using `secreq` as your SSH agent** | [ssh-agent.md](./ssh-agent.md) — config, `ssh-setup` wiring, the key-custody tradeoff. |
...
- **[ssh-agent.md](./ssh-agent.md)** — the provenance-aware SSH agent:
  the `ssh` config block, `ssh-setup` wiring (`~/.ssh/config` or shell
  rc), the per-anchor TTL, and the key-custody downgrade vs.
