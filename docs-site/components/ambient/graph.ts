/**
 * The ambient layer's geometry: a call graph, generated procedurally.
 *
 * Renderer-agnostic on purpose. Both the SVG and the canvas backend import
 * this, so switching between them compares two ways of *drawing* the same
 * graph rather than two different graphs.
 *
 * secreq's entire subject is a request travelling up through processes to
 * reach a decision. So this generates that shape — seeded, so a resize
 * re-derives the same graph instead of reshuffling it.
 *
 ...
 * Axis-aligned segments read as more of the page's own rules, which is the one
 * thing a background layer must not do; angles nothing else on the page uses
 * can never be mistaken for structure. Links are barred from near-horizontal
 * for the same reason.
 */

/* ── Timing ──────────────────────────────────────────────────────────────
   Shared by both backends. The SVG one spends these as CSS keyframe
   percentages and animation delays; the canvas one evaluates them directly
   against the clock. They have to agree or the two renderers stop being
   comparable. */

/** One full pulse cycle, in seconds. */
export const PULSE_PERIOD = 15;

/** Share of the cycle a pulse spends travelling; the rest is the pause. */
export const PULSE_TRAVEL_FRACTION = 0.52;

/** Seconds a pulse spends travelling. */
export const PULSE_TRAVEL = PULSE_PERIOD * PULSE_TRAVEL_FRACTION;

/** Share of the cycle a node's arrival ripple occupies. */
export const ARRIVAL_FRACTION = 0.09;
...
export function buildGraph(width: number, height: number): Graph {
  ...
  const connected = new Set(
    segments.map((s) => `${Math.min(s.from, s.to)}:${Math.max(s.from, s.to)}`),
  );
  ...
  const canLink = (i: number, j: number): boolean => {
    ...
    if (Math.abs(Math.PI - Math.abs(Math.atan2(b.y - a.y, b.x - a.x))) < MIN_LINK_ANGLE)
      return false;
  ...
  const route = (start: number): number[] => {
    ...
    for (let step = 0; step < 12; step++) {
      ...
      const pool = climbing.length && (rand() < 0.8 || !sideways.length) ? climbing : sideways;
