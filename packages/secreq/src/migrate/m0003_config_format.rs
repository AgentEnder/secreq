...
/// and no sigils, so this is a pure format change — except for nulls.
///
/// `RuleMatch`'s optional clauses carry `default` but no `skip_serializing_if`,
/// so the most ordinary rule there is — match a wrap, nothing else — is written
/// as `"argv": null, "ancestor": null, "cwd": null`. TOML has no null, and
/// `toml_edit`'s serializer reports one as an "unsupported unit type", failing
/// the whole migration on a file `secreq` itself wrote.
///
/// Dropping the key is the same answer [`json_to_item`] already gives on the
/// config side, and it round-trips: `#[serde(default)]` reads an absent key
/// back as the `None` it came from.
...
///
/// Deliberately not `json_to_item`: that one builds a `toml_edit` tree of
/// inline tables, and the rules file wants the serializer's own layout.
...
mod tests {
    ...
    /// unset clauses as explicit nulls — `RuleMatch`'s options carry `default`
    /// but no `skip_serializing_if`. TOML has no null, so handing one to the
    /// serializer is an "unsupported unit type" and the whole migration fails.
    ///
    /// This is not an exotic file: it is what `secreq` writes for the most
    /// ordinary rule there is, which is how it reached a real `~/.secreq`
    /// while every fixture here used fully-populated rules.
    #[test]
    fn a_rule_with_unset_match_clauses_converts() {
        let out = convert_rules(
            r#"{
                rules: [{
                    id: "0a1b2c3d4e5f",
                    name: "a rule",
        ...
        assert!(out.contains(r#"wrap = "gh""#), "{out}");
        // The clauses are dropped, not rendered as empty strings — an empty
