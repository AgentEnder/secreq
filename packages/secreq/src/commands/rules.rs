...
pub fn rules_add_wasm(
    ...
    // A wrap scope is pure config policy. Validate it before touching the
    // module so a typo gets the actionable error even when the file path is
    // also bad, and a missing config behaves as the empty scope domain.
