...
| `51-rules-stale-mutation` | `rules_tab_stale_mutation_rejected` | A same-rule stale-write conflict surfaced above the still-current deny rule. The window says the change was rejected and tells the operator to reload instead of silently replacing newer policy. |
