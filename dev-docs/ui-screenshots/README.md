...
| `24-ssh-sign-pending.png` | `ssh_sign_pending` | An SSH-agent sign prompt (the in-process consent the daemon raises on a cache-miss `SIGN_REQUEST`). The card headlines "SSH key request" + the identity name (`github`), shows the key's SHA256 fingerprint on a `sign with …` line and the configured `$reason` beneath it, and renders the caller chain (git under zsh) via the shared process tree. Distinct from a wrap card: no "secrets to inject" list and no "gate only" marker — an SSH sign releases a signature, not an env var. Approve/Deny/Approve-all share the wrap card's decision semantics. |
