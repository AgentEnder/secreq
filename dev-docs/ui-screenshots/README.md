...
| `24-ssh-sign-pending.png` | `ssh_sign_pending` | An SSH-agent sign prompt (the in-process consent the daemon raises when no session grant covers a `SIGN_REQUEST`). The card headlines "SSH key request" + the identity name (`github`), shows the key's SHA256 fingerprint on a `sign with …` line and the configured `$reason` beneath it, and renders the caller chain (git under zsh) via the shared process tree. Distinct from a wrap card: no "secrets to inject" list and no "gate only" marker — an SSH sign releases a signature, not an env var. Its dedicated action row carries the four session choices: **Approve once · Approve 30m · Approve all keys 30m · Deny** (the two 30m choices remember a TTL-bounded session grant scoped to this anchor — one key, or all keys). |
| `25-badge-one-pending.png` | `badge_one_pending` | The always-on-top pending badge (`render_badge`) with a single request — the singular "1 pending" branch. A compact alert-red pill with a bright indicator dot, rendered in its own borderless `secreq pending-badge` window that floats over other apps so a backgrounded consent window can't be forgotten. Not the consent panel: its own minimal harness path (no daemon state / audit / tabs). |
| `26-badge-three-pending.png` | `badge_three_pending` | The pending badge with the common multi-request count ("3 pending"). The whole pill is one click target; a click sends `RaiseConsentRequested` to bring the consent window forward. |
| `27-audit-tab-abandoned.png` | `audit_tab_abandoned_row` | Audit tab showing an `abandoned` row (`gh pr checkout 9420`) — a wrap that exited before the user decided, so the daemon reaped the ask and logged it itself. The muted grey "abandoned" verdict pill reads as a non-event, visually distinct from the red "denied" pill on the neighbouring `aws` row and a real approve. |
| `run-consent.png` | `run_consent_card` | The `secreq run` consent card — free-form command, ambient-sourced secrets, no remembered approval. The dedupe identity is the fixed `"run"` so the card headlines the typed command (`./deploy.sh --prod`), and the two secrets come from scanning the ambient env for `secret://` refs: `DATABASE_URL` (op) + `STRIPE_KEY` (keychain). `allow_remember = false` (no visual signature) keeps the fixture faithful to the production `run` ask. |
| `run-session-card.png` | `run_session_card` | A coalesced `run` *session*: three sibling `run` asks under the same process merge into one card whose secret list is the union of what each requested, each entry carrying its `← command` provenance (`DATABASE_URL ← ./migrate`, `STRIPE_KEY`/`REDIS_URL ← ./worker`). Three waiters → the `×3 waiting` pill; one `Approve all` / `Deny` covers the whole session. |
